...
const CartPage = () => {
  ...
  const [modalConfig, setModalConfig] = useState({ amount: 0, items: [], isSingleItem: false });
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        {cartItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-28 text-center">
            ...
            <p className="text-xl font-bold text-slate-700 dark:text-slate-300 mb-2">Your cart is empty</p>
            <p className="text-slate-400 dark:text-slate-500 text-sm mb-6">Add some products to get started</p>
            <Link to="/" className="px-6 py-2.5 bg-indigo-600 text-white rounded-xl font-semibold text-sm hover:bg-indigo-500 transition-colors shadow-lg shadow-indigo-600/20">
              Browse Products
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => dispatch(updateQuantity({ id: item.id, amount: -1 }))}
                      className="w-7 h-7 flex items-center justify-center rounded-lg bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-white/10 font-bold text-sm transition-colors">−</button>
                    <span className="w-6 text-center font-bold text-sm text-slate-900 dark:text-white">{item.quantity}</span>
                    <button onClick={() => dispatch(updateQuantity({ id: item.id, amount: 1 }))}
                      className="w-7 h-7 flex items-center justify-center rounded-lg bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-white/10 font-bold text-sm transition-colors">+</button>
                  </div>
                  {/* Actions */}
            ...
            <div className="lg:col-span-1">
              <div className="bg-white dark:bg-slate-800/60 border border-slate-100 dark:border-white/5 rounded-2xl p-6 sticky top-24">
                <h3 className="font-bold text-slate-900 dark:text-white text-base mb-5">Order Summary</h3>
                <div className="space-y-3 text-sm mb-5">
                  <div className="flex justify-between text-slate-500 dark:text-slate-400">
                    <span>Subtotal ({totalItems} items)</span>
                    <span className="font-medium text-slate-700 dark:text-slate-300">₹{total.toLocaleString("en-IN")}</span>
                  </div>
                  <div className="flex justify-between text-slate-500 dark:text-slate-400">
                    <span>Shipping</span>
      ...
      <PaymentModal
        ...
        isSingleItem={modalConfig.isSingleItem}
      />
    </div>
  );
};
